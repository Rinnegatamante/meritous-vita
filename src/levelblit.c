...
void VideoUpdate()
{
	...
	if (WriteBitmaps) {
		if ((bmp >= WB_StartRange)&&(bmp < WB_EndRange)) {
			sprintf(bmp_name, "ux0:data/meritous/bmp%d.bmp", bmp);
...
// switch
	...
	asceai = IMG_Load("ux0:data/meritous/dat/i/asceai.png");
	wm_icon = IMG_Load("ux0:data/meritous/dat/i/icon.png");
...
//	screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 8, SDL_SWSURFACE | (SDL_FULLSCREEN * fullscreen));
	...
	wm_mask_file = fopen("ux0:data/meritous/dat/d/icon_bitmask.dat", "rb");
	...
	while (executable_running) {
		...
		title = IMG_Load("ux0:data/meritous/dat/i/title.png");
		title_pr = IMG_Load("ux0:data/meritous/dat/i/title.png");
		...
		while (on_title) {
			...
			draw_text(17, 210, "RaveHeart - XandridFire", 225 + sin((float)ticker_tick / 15)*30);
		...
		if (executable_running == 1) {
			...
			if ((option == 0) && can_continue) {
				DungeonPlay("ux0:data/meritous/SaveFile.sav");
			} else {
				if (option == (0 + can_continue)) {
					training = 0;
					DungeonPlay("");
...
void DrawMeter(int x, int y, int n)
{
	...
	if (meter == NULL) {
		meter = IMG_Load("ux0:data/meritous/dat/i/meter.png");
			...
			if (player_room == agate_knife_loc) {
				{
					...
					SDL_Rect draw_to;
					if (agate_knife == NULL) {
						agate_knife = IMG_Load("ux0:data/meritous/dat/i/agate.png");
					...
					if (dist(player_x, player_y, xpos, ypos) < 20) {
						...
						SND_Pos("ux0:data/meritous/dat/a/crystal2.wav", 128, 0);
...
void HandleEvents()
{
	...
	key_held[K_UP] = (pad.buttons & SCE_CTRL_UP) || (pad.ly < 80);
	key_held[K_DN] = (pad.buttons & SCE_CTRL_DOWN) || (pad.ly > 180);
	key_held[K_LT] = (pad.buttons & SCE_CTRL_LEFT) || (pad.lx < 80);
	key_held[K_RT] = (pad.buttons & SCE_CTRL_RIGHT) || (pad.lx > 180);
	if (pad.buttons) {
		CancelVoluntaryExit();
		if (pad.buttons & SCE_CTRL_START) game_paused ^= 1;
...
//					tele_select = 0;
		...
		if (pad.buttons & SCE_CTRL_CROSS) {
			if (map_enabled && tele_select) {
				...
				game_paused = 0;
				tele_select = 0;
			} else key_held[K_SP] = 1;
		} else key_held[K_SP] = 0;
		if (pad.buttons & SCE_CTRL_SQUARE) enter_pressed = 1;
		...
		if (pad.buttons & SCE_CTRL_LTRIGGER) ShowHelp();
	} else key_held[K_SP] = 0;
...
void DrawLevel(int off_x, int off_y, int hide_not_visited, int fog_of_war)
{
	...
	if (tiles == NULL) {
		tiles = IMG_Load("ux0:data/meritous/dat/i/tileset.png");
		fog = IMG_Load("ux0:data/meritous/dat/i/tileset.png");
...
void DrawPlayer(int x, int y, int pl_dir, int pl_frm)
{
	...
	if (playersprite == NULL) {
		playersprite = IMG_Load("ux0:data/meritous/dat/i/player.png");
...
void text_init()
{
	FILE *font_data_file;
	int chr, x, y;
	font_data_file = fopen("ux0:data/meritous/dat/d/font.dat", "rb");
		...
		while (!given_treasure) {
			...
			switch (treasure) {
				case 0:
					...
					given_treasure = 1;
					SND_Pos("ux0:data/meritous/dat/a/tone.wav", 128, 0);
					break;
				case 1:
					if (player_shield < 25) {
						specialmessage = 10;
	...
	switch (tile) {
		...
		case 28:
			...
			if (player_gems >= UpgradePrice(0)) {
				...
				player_shield += 1;
				SND_Pos("ux0:data/meritous/dat/a/crystal.wav", 128, 0);
			}
			break;
		case 29:
		...
		case 32:
			CrystalSummon();
			SND_Pos("ux0:data/meritous/dat/a/crystal.wav", 80, 0);
			break;
		default:
			break;
	}
...
void DrawArtifacts()
{
	...
	if (artifact_spr == NULL) {
		artifact_spr = IMG_Load("ux0:data/meritous/dat/i/artifacts.png");
